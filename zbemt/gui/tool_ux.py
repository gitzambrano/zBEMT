...
class ToolWorkflowHeader(QFrame):
    ...
    def __init__(self, tabs, steps, parent: QWidget | None = None):
        ...
        # The application theme styles the Tool header as a dark strip.  A
        # local, theme-independent information strip keeps the current action
        # readable in both light and dark application palettes instead of
        # relying on palette roles that the global stylesheet can override.
        self.guidance.setStyleSheet(
            "QLabel { background-color: #f4f6f8; color: #20242a; "
            "border: 1px solid #d7dbe0; border-radius: 4px; "
            "padding: 5px 8px; }"
        )
