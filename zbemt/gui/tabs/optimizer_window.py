...
class OptimizerWindow(QWidget):
    ...
    def __init__(self, state: AppState, parent=None):
        ...
        self.pages.addTab(in_scroll_area(self._build_definition_page()), "Study")
    ...
    def _build_definition_page(self) -> QWidget:
        ...
        self._advanced_search_widgets = []
        for r, (label, widget) in enumerate(rows):
            label_widget = QLabel(label)
            grid.addWidget(label_widget, r, 0)
            grid.addWidget(widget, r, 1)
            self._advanced_search_widgets.extend((label_widget, widget))
        ...
        search_box.toggled.connect(self._set_advanced_search_visible)
        self._set_advanced_search_visible(False)
    ...
    def _set_advanced_search_visible(self, visible: bool):
        """Collapse numerical search tuning until the user asks for it."""
        for widget in getattr(self, "_advanced_search_widgets", ()):
            widget.setVisible(bool(visible))
        if hasattr(self, "advanced_search_box"):
            self.advanced_search_box.updateGeometry()
