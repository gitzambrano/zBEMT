...
class TransientWindow(QWidget):
    ...
    def __init__(self, state: AppState, parent=None):
        ...
        self.pages.addTab(in_scroll_area(self._build_trajectory_page()), "Trajectory")
    ...
    def _build_trajectory_page(self) -> QWidget:
        ...
        # Keep all seven engineering headings readable at notebook width.
        # The editor column may grow; the settings pane still has enough
        # room at the supported 1100 px review width.
        self.points_table.setMinimumWidth(520)
    ...
    def _refresh_preview(self):
        from zbemt.studies import _maneuver_samples
        definition = self._current_definition()
        if len(definition.points) < 2:
            self.preview_canvas.show_message(
                "Preview appears after you build a ramp or add at least two trajectory points.")
            return
        try:
