...
BLOCK_HELP: dict[str, dict] = {
    "operation_mode": {
        ...
        "body": [
            ...
            "<b>Propeller convention.</b> The rotational frequency n = rpm/60 and the diameter D = 2R are the reference scales: C<sub>T</sub> = T/(ρ·n²D⁴), C<sub>P</sub> = P/(ρ·n³D⁵), with advance ratio J<sub>x</sub> = V<sub>x</sub>/(nD). Efficiency is η = J<sub>x</sub>·C<sub>T</sub>/C<sub>P</sub> — the ratio of useful propulsive power T·V<sub>x</sub> to shaft power, which is identically zero at static thrust and peaks at a finite J<sub>x</sub>.",
    ...
    "mesh_atmosphere": {
        ...
        "body": [
            "The disk is discretized into Ne radial × Npsi azimuthal stations; the solver evaluates all Ne×Npsi elements at once per iteration. Δψ = 360°/Npsi. Npsi = 1 means axisymmetric (hover/axial); resolving advancing/retreating asymmetry needs enough harmonics — 24 is a minimum, 72–144 for high μ or cyclic pitch. Radially, 30–50 is usually converged; the reference production mesh is 120×180.",
    ...
    "inflow": {
        ...
        "body": [
            ...
            "K<sub>y</sub> is quasi-constant (depends only on μ): an approximately uniform left/right tilt that Coleman ignores. Both coefficients are empirical fits — K<sub>x</sub> and K<sub>y</sub> follow from the μ and λ already solved, with no tunable parameter.",
    ...
    "rotational_augmentation": {
        ...
        "body": [
            ...
            "Read the three factors: (c/r)² makes it a root effect — it collapses outboard; λ<sub>r</sub>²/(1 + λ<sub>r</sub>²) → 1 when rotation dominates (root, hover) and → 0 when the axial component dominates (tip, high μ), recovering pure 2D at the tip; and the bracket is the <b>lift deficit</b> the static polar has already lost to separation, so the correction is identically zero while the flow is attached and grows only past stall. g(α) fades the model out above ≈ 60°.",
    ...
    "pitt_peters": {
        ...
        "body": [
            ...
            "with α* = arctan(λ/μ) the wake skew angle. That off-diagonal term is the wake tilt — same role as K<sub>x</sub> in Coleman/Drees, but emerging from physics.",
    ...
    "run_case": {
        ...
        "body": [
            "A case is (μ<sub>x</sub> or J<sub>x</sub>, V<sub>z</sub>, collective, rpm). From it: Ω = 2π·rpm/60, tip speed ΩR, and the two element velocities U<sub>T</sub> = Ωr + V<sub>x</sub>·sin ψ and U<sub>P</sub> = V<sub>z</sub> + v<sub>i</sub> + (radial/coning terms), with W = √(U<sub>T</sub>² + U<sub>P</sub>²).",
