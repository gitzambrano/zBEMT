...
def project_outputs_dir(project_or_path, create: bool = False) -> str:
    ...
    exists to not have."""
    project_path = getattr(project_or_path, "path", project_or_path)
    if not project_path:
        destination = Path("outputs")
    else:
        destination = default_project_paths(str(project_path))["outputs"]
    if create:
...
def open_project(path: str) -> Project:
    ...
    if paths["config"].exists():
        import json
        from .models import (_from_jsonable_scalar, warn_unknown_keys,
                             migrate_config_raw)
        ...
        config = migrate_config_raw(_from_jsonable_scalar(config))
        warn_unknown_keys(BEMTConfig, config, context=str(paths["config"]))
    ...
    # default (folder name), which stays backward compatible.
    name = Path(path).name
    ...
    if paths["legacy_batch"].exists():
        ...
        legacy = load_bemt(BatchDefinition, paths["legacy_batch"], is_propeller)
        has_content = bool(legacy.conditions) or bool(legacy.sweep_params)
        if has_content and legacy not in batches:
...
# GUI ("Batches defined in this project" and "Saved cases" list) and by the
...
# =============================================================================
# Validation (checking mutually exclusive or redundant combinations, see
# validation.py). It does not run the engine nor touch disk, but lives in
...
def validate_project(project: Project, conditions=None) -> list[validation.Issue]:
    ...
    engine's non-dimensionalization; neither is visible just by
...
# Case and Run Batch fields without importing bemt.py directly (hard rule of
...
def export_polar_table(polar_slices: list[PolarSlice], path: str) -> Path:
    ...
    already know how to read back. This closes the loop requested in
    Phase 7: generate via NeuralFoil, save to disk, then reimport like
    any other table (useful for versioning, reusing across projects, or
    editing before reimporting)."""
    return airfoils.export_polar_slices_csv(polar_slices, path)


# =============================================================================
# Persistence and post-processing (explicit, separate from running)
# =============================================================================
...
#: ASCII transcription of the symbols `studies.condition_name` uses
...
def sanitize_filename(name: str, used: Optional[set] = None) -> str:
    ...
    - transcribes to ASCII the symbols generated by
      `studies.condition_name` (``μ=0.1, α=-10°`` -> ``mu_x=0.1, alpha=-10deg``);
      see `_ASCII_TRANSCRIPTION`;
    ...
    name = "".join(_ASCII_TRANSCRIPTION.get(c, c) for c in (name or ""))
    clean = "".join("-" if c in _ILLEGAL_CHARACTERS else c for c in name)
    ...
    if used is not None:
        base, n = clean, 2
        while clean.lower() in {u.lower() for u in used}:
...
def filename_map(names) -> dict:
    ...
    `_3`, `_4` for the SAME condition. That leaves files scattered around
    instead of overwritten, which is just another form of wrong.
    ...
    IDENTICAL name (the common case, since the GUI names every Run Case case
    ...
    `condition_filenames`, which returns a LIST by position."""
    used: set = set()
    return {name: sanitize_filename(name, used) for name in names}


def condition_labels(results_list) -> list:
    """``["Case 1", "Case 2", ...]``: a UNIQUE, numbered label per
    ...
    rows, with no way to say "row 3" or cross-reference a row with a
    figure.
    ...
    names = [str(getattr(r, "condition_name", "") or "").strip()
             for r in results_list]
    distinct = all(names) and len(set(names)) == len(names)
    return [f"Case {i} - {name}" if distinct else f"Case {i}"
            for i, name in enumerate(names, start=1)]


def condition_filenames(results_list) -> list:
    """Filename per condition, indexed by POSITION in
    ``results_list`` (not by name, see `filename_map`).

    Starts from the numbered label, so two conditions never share a
    file even when they share a name. `used` still resolves the
    residual collision of two labels that sanitize to the same text."""
    used: set = set()
    return [sanitize_filename(label, used) for label in condition_labels(results_list)]


def _condition_filename(condition_name: str) -> str:
...
def _reverse_flow_mask(result) -> bool:
    ...
    the place, because it is the single point of reading."""
...
def _result_plot_path(plots_dir: Path, layout: str, base: str, condition_name: str) -> Path:
    ...
    `_path_without_overwrite`, which appends `" (2)"`, `" (3)"`...
    Without this, exporting the disk maps twice to the SAME folder would
    silently erase the first export. This is the same class of data loss as
    the bug where N conditions with the same name all landed in the same
    file (see `condition_filenames`). The suffix does NOT
    ...
    """
    cond = _condition_filename(condition_name)
    if layout == "per_case":
        d = plots_dir / cond
        d.mkdir(parents=True, exist_ok=True)
        return _path_without_overwrite(d / f"{base}.png")
    return _path_without_overwrite(plots_dir / f"{base}_{cond}.png")


def _path_without_overwrite(path: Path) -> Path:
...
def _excel_number_formatter(decimal: str):
    ...
    used on screen, see ``RunCaseTab._format_value``) switches to
...
def export_results_tabular(results, outdir: str, sep: str = ",",
    ...
    previous export (see ``_excel_number_formatter``/``_path_without_overwrite``).
...
def _swept_axis(results_list) -> str:
    ...
    for axis in _SWEEP_AXES:
        key = plots_module._AXIS_TO_SUMMARY_KEY.get(axis, axis)
        values = {r.summary.get(key) for r in results_list}
...
def _group_by_reynolds(conditions):
    ...
    `_export_airfoil_polars`."""
    if not conditions or conditions == [None]:
        return [(None, [None])]
    reynolds = {c.get("reynolds") for c in conditions if isinstance(c, dict)}
    machs = {c.get("mach") for c in conditions if isinstance(c, dict)}
    ...
    for re_val in sorted(reynolds):
        ...
        if subset:
            groups.append((re_val, subset))
    return groups


def _export_airfoil_polars(project, plots_dir: Path, results_list: list,
    ...
    sections = list(project.airfoil_sections or [])
    if len(sections) >= 2:
        targets = [(f"_r{s.r_norm:.2f}".replace(".", "p"), s) for s in sections]
    else:
        targets = [("", project.airfoil)]

    use_comp = bool((project.config or {}).get("use_compressibility", False))
    written: list = []
    for suffix, af in targets:
        if af is None:
            continue
        try:
            conditions = airfoils.unique_conditions(af) or [None]
            # Grouping by Reynolds: when the table varies in Reynolds
            # AND in Mach (the actual reported case, for example
            # 3 Re x 3 M), overlaying EVERYTHING on the same chart gives
            # 9 curves that are impossible to read. Splitting into ONE
            # figure PER Reynolds, with one curve per Mach inside each,
            # shows the variation with Mach (for a fixed Re) on one page
            # and the variation with Re on another. If only Re varies
            # (without Mach, or vice versa), a SINGLE figure comes out
            # with one curve per value, the same behavior as before.
            groups = _group_by_reynolds(conditions)
        except Exception:
            continue
        if not groups:
            continue

        for rey, subset in groups:
            try:
                curves = airfoils.preview_polar_multi(
            ...
            if not curves:
                continue
            suffix_group = suffix + (f"_Re{rey:.0f}" if rey is not None else "")
            ...
            for base, draw in (
                ...
                try:
                    for c in curves:
                        draw(ax, c)
                    if rey is not None:
                        ax.set_title(f"{ax.get_title()} — Re={rey:.3g}")
                    fname = plots_dir / f"{base}{suffix_group}.png"
                    fig.tight_layout()
        ...
        if geom_2d is not None and getattr(geom_2d, "x", None):
            fname = plots_dir / f"airfoil_profile{suffix}.png"
            try:
...
def export_results(results, outdir: str, plots: Optional[list[str]] = None,
    ...
    - ``"disk_map_<field>"`` (for example ``disk_map_CT``,
      ``disk_map_alpha``) — only the .png of that specific field, one PER
      condition.
    ...
    - ``"rotor_3d"``       — 3D screenshot of the rotor/blade (via
      PyVista, ``visualization.py``), from ``project.geometry``, so it is
      only available if PyVista is installed. It **requires** that
    ...
    - ``"load_3d"`` or ``"load_3d_<field>"`` — shortcut for
      ...
      more like "load distribution", per Section 8.5 of the plan).

    Unknown kinds are silently ignored: the GUI only offers the valid
    ones, and scripts get an empty file list for that item. The ``*_3d``
    kinds are also silently ignored (no error) when PyVista is not
    installed, so the platform keeps working without that optional
    dependency (see docs/plano.md Section 4, ``visualization.py``).

    ``project``: optional. It is only needed if ``plots`` includes
    ...
    # figures to the same file (see `condition_filenames`).
    filenames = condition_filenames(results_list)
    ...
    if save_csv:
        ...
        csv_path = _path_without_overwrite(out / "results.csv")
    ...
    if plots:
        ...
        for kind in plots:
            if kind == "performance":
                fname = _path_without_overwrite(plots_dir / "performance.png")
                plots_module.plot_performance(results_list, fname=str(fname))
                written.append(str(fname))

            elif kind == "convergence":
                for i, r in enumerate(results_list):
                    fname = _result_plot_path(plots_dir, layout, "convergence", filenames[i])
                    plots_module.plot_convergence(r, fname=str(fname))
                    written.append(str(fname))

            elif kind == "comparison":
                fname = _path_without_overwrite(plots_dir / "solver_comparison.png")
                ...
                for i, r in enumerate(results_list):
                    if field not in r.maps:
                        continue
                    fname = _result_plot_path(plots_dir, layout, f"{prefix}_{field}", filenames[i])
                    if is_load:
                ...
                # complete picture.
                axis = _swept_axis(results_list)
                fname = plots_dir / f"coefficients_vs_{axis}.png"
                plots_module.plot_coefficients_vs_axis(results_list, axis=axis, fname=str(fname))
                written.append(str(fname))
                ...
                # the GUI to see it.
                written += _export_airfoil_polars(
                ...
                for i, r in enumerate(results_list):
                    if not r.maps:
                        continue
                    mask_reverse = _reverse_flow_mask(r)
                ...
                for i, r in enumerate(results_list):
                    ...
                    fname_grid = _result_plot_path(plots_dir, layout, "disk_map_grid", filenames[i])
                    ...
                    for field in plots_module._DISK_GRID_FIELDS:
                        fname = _result_plot_path(plots_dir, layout, f"disk_map_{field}", filenames[i])
                ...
                fname = _path_without_overwrite(plots_dir / "rotor_3d.png")
...
# which opens in any browser and survives being sent by email. There is
# no folder of PNGs that gets separated from the HTML.
#
...
_REPORT_CSS = """
  ...
  * { box-sizing: border-box; }
  ...
  /* Blocks side by side: a transposed quantity|value table becomes very
     tall in a single column; in N blocks it fits on the screen with no
     sideways scrolling */
  .blocks { display: flex; flex-wrap: wrap; gap: 0 24px; align-items: flex-start;
            justify-content: center; }
  .blocks table { width: auto; flex: 1 1 260px; }
  .blocks td:last-child, .blocks th:last-child { text-align: right; }
  /* A batch summary matrix can have more columns than fit the width of
     the report even after `_MAIN_COLUMNS`; without this the wide table
     stretches the WHOLE PAGE (sideways scrolling for the whole report)
     instead of only itself. */
  .tablewrap { overflow-x: auto; margin: 12px 0; border: 1px solid #dde1e6;
               border-radius: 4px; }
  /* `table-layout: fixed` + an explicit column width = EVERY column with
     the SAME narrow width, instead of each one stretching to its own
     content (which gave irregular column widths and a table much wider
     than necessary). */
  ...
  /* An INSTANT tooltip. The native `title=` appears only after
     ...
     one-letter columns each, that delay turns reading the table into a
     queue of waits. This is NO LONGER an `::after` positioned with
     `position: absolute` inside the `<th>` itself -- that `<th>` lives
     inside `.tablewrap`, whose
  ...
  /* Index of the satellite reports. A heavy section lives in its own file,
     as `_FIGURES_BEFORE_OWN_FILE` describes, and is linked from
     here. */
  ul.satellites { list-style: none; padding: 0; display: flex; flex-wrap: wrap;
                  gap: 10px; }
  ul.satellites li { flex: 1 1 300px; border: 1px solid #dde1e6; border-radius: 4px;
                     padding: 10px 14px; background: #fafbfc; }
  ul.satellites a { color: var(--accent); font-weight: 600; text-decoration: none; }
  ul.satellites a:hover { text-decoration: underline; }
  ul.satellites .cnt { color: #6b7280; font-size: 13px; }
  ...
  .note { background: #fff8e1; border-left: 4px solid #ffb300; padding: 10px 14px;
  ...
  @media (prefers-color-scheme: dark) {
    ...
    .note { background: #2a2416; color: #e6e6e6; }
    ...
    ul.satellites li { background: #1a1d22; border-color: #33383f; }
  }
  @media print {
...
(function () {
  ...
  document.addEventListener("mouseover", function (ev) {
    var target = ev.target.closest(".tt");
    if (!target) return;
    var tipText = target.getAttribute("data-tip");
    if (!tipText) return;
    pop.textContent = tipText;
...
#: `_sorted_keys`) and by the Run Case tab (which groups the same
#: keys, `run_case.py`'s `_GROUPS_ROTOR`/`_GROUPS_PROPELLER`, in the same
#: relative order, with a test locking this down in both files).
#:
...
#: implementation accident, that nobody decided, and that the Run Case
    ...
    #: `_sorted_keys` returns an empty `remaining` and the three
...
#: fall after about 40 output columns.
_MAIN_COLUMNS = (
...
#: training needs the tooltip anyway, so both come out ahead.
...
_COLUMN_SYMBOL = {
    ...
    # merged in below (`_axis_symbols`). That module is the single source
    ...
    # symbol (not the raw field name, see `_column_header`) plus the
...
def _axis_symbols(is_propeller: bool) -> dict:
    ...
    Input-only aliases (`alpha_deg`, `alpha_disk`) are left out, because they never
    appear as a `Results.summary` key."""
    return {key: (nomenclature.symbol_html(key, is_propeller),
                  nomenclature.description_html(key, is_propeller))
            for key, q in nomenclature.QUANTITIES.items() if not q.alias_of}


_COLUMN_SYMBOL.update(_axis_symbols(False))


def _description_with_symbols(text: str) -> str:
...
#: symbol in `zbemt/nomenclature.py`, and `_column_unit` consults that
...
#: loses its unit.
_COLUMN_UNITS.update({key: nomenclature.unit(key)
                      for key, q in nomenclature.QUANTITIES.items()
                      if not q.alias_of})
...
# Section 6c). On a helicopter that coincides with the vehicle's axes. On a
# PROPELLER it does not: the shaft points forward, and in cruise the whole
# airspeed lies along it. Therefore the table used to label the flight velocity
...
def summary_symbols(is_propeller: bool = False) -> dict:
    ...
    In rotor mode it returns `_COLUMN_SYMBOL` unchanged. In propeller
    mode it applies the axis-letter rotation from `nomenclature` plus the
    two non-axis columns whose prose differs
    (`_COLUMN_SYMBOL_PROPELLER`). It is the SINGLE source for the report,
    the GUI table, and field selectors: the three surfaces need to say the
    SAME thing about the same column."""
    if not is_propeller:
        return _COLUMN_SYMBOL
    table = dict(_COLUMN_SYMBOL)
    table.update(_axis_symbols(True))
    table.update(_COLUMN_SYMBOL_PROPELLER)
    return table


def summary_symbol(key: str, is_propeller: bool = False) -> tuple:
    """``(symbol, description)`` of a column, or ``(key, "")`` if it is
    not in the table (never leaves the interface without a header)."""
    return summary_symbols(is_propeller).get(key, (key, ""))


def _column_header(key: str, is_propeller: bool = False) -> str:
    """`<th>` of the SYMBOL row, with an instant tooltip.

    The tooltip is CSS (`data-tip` + `::after`, see `_REPORT_CSS`), not the
    native `title=`: the native one only appears after about 1 second stopped over the
    ...
    test."""
    symbol, description = summary_symbols(is_propeller).get(
        key, (key.removeprefix("cfg_"), key))
    description = _description_with_symbols(description)
    # `data-tip` becomes CSS content: double quotes would break the attribute.
    tip = description.replace('"', "&quot;")
    return f'<th class="tt" data-tip="{tip}">{symbol}</th>'


def _column_unit(key: str) -> str:
    """`<th>` of the header's second row: the unit."""
    unit = _COLUMN_UNITS.get(key, "")
    return f'<th class="un">{f"[{unit}]" if unit else ""}</th>'
...
#: The numbers come from the usable width (minus padding) at about 6 px/character in the
#: table's 12 px body.
_CHARACTERS_PER_CELL = 10
_CHARS_FIRST_COLUMN = 18


def _symbols_to_text(html: str) -> str:
...
def _html_cell(text: str, first: bool = False, html: str | None = None) -> str:
    ...
    native `title=` only appears after about 1 second, and checking half a dozen
    ...
    limit = _CHARS_FIRST_COLUMN if first else _CHARACTERS_PER_CELL
    display = html if html is not None else text
    tip = _symbols_to_text(display)
    if len(text) <= limit:
        return f"<td>{display}</td>"
    return f'<td class="tt" data-tip="{tip.replace(chr(34), "&quot;")}">{display}</td>'


def _table_html(headers: list, rows: list, *, vertical: bool = False,
    ...
    (`_MAIN_COLUMNS`). Without the wrapper, the wide table stretches the
    ...
    raw field names such as ``reverse_flow_model`` do not fit a
    74px column horizontally) rotates the header text 90° via
    ``.tablewrap.vert`` (see `_REPORT_CSS`). The first column
    (``condition``) is left out: it is short and sticky, and rotating it would
    only get in the way."""
    css_class = "tablewrap vert" if vertical else "tablewrap"
    header_row = [_column_header(h, is_propeller) for h in headers]
    if vertical and header_row:
        header_row[0] = header_row[0].replace('class="tt"', 'class="tt horiz"', 1)
    html = [f'<div class="{css_class}"><table><thead>',
            "<tr>" + "".join(header_row) + "</tr>",
            "<tr>" + "".join(_column_unit(h) for h in headers) + "</tr>",
            "</thead><tbody>"]
    for row in rows:
        cells = []
        for j, c in enumerate(row):
            text = str(c)
            with_symbols = _description_with_symbols(text) if j == 0 else None
            cells.append(_html_cell(text, first=(j == 0), html=with_symbols))
        html.append("<tr>" + "".join(cells) + "</tr>")
    html.append("</tbody></table></div>")
    return "".join(html)


def _quantity_value_blocks(pairs: list, columns: int = 3,
                           is_propeller: bool = False) -> str:
    ...
    with about 70 quantities, the reader scrolled horizontally endlessly and never saw
    ...
    if not pairs:
        return ""
    per_block = -(-len(pairs) // columns)      # ceiling division
    blocks = [pairs[i:i + per_block] for i in range(0, len(pairs), per_block)]
    parts = ['<div class="blocks">']
    for block in blocks:
        parts.append("<table>")
        parts.append("<tr><th>quantity</th><th>value</th></tr>")
        for key, value in block:
            symbol, description = summary_symbols(is_propeller).get(key, (key, key))
            parts.append(f'<tr><td title="{description}">{symbol}</td><td>{value}</td></tr>')
        parts.append("</table>")
    parts.append("</div>")
    return "".join(parts)
...
def _format_value(value) -> str:
    ...
    guarantees there is none. Below the floor, it is ZERO: 1e-12 is about 9
    orders of magnitude below the smallest physically significant coefficient that
...
def _rotor_rpm_is_redundant(results_list: list) -> bool:
    ...
    divergence, which is exactly when it would matter."""
    for r in results_list:
...
def propeller_mode(results_list: list, project: Optional[Project] = None) -> bool:
    """The mode in which these results were run.

    The authoritative answer is the project (it is what the user edited). In its
    absence, the answer is the `cfg_is_propeller` echo that `export_settings`
    writes into each `Results.summary`. With neither of the two, the answer is
    rotor, which is `BEMTConfig`'s default and what the table has always shown.
    ...
    since three different answers about the same `Results` would be worse than
    one wrong one."""
...
def _sorted_keys(results_list: list, is_propeller: Optional[bool] = None) -> tuple:
    ...
    (see `_rotor_rpm_is_redundant`).

    ``is_propeller`` (``None`` = deduce from `propeller_mode`) changes TWO things
    and neither of them is a value: the order of the first columns (a propeller
    reads by axial advance) and which of the two angles is shown. Both are
    read from `nomenclature`, so the table, the plots and the `.bemt` writer agree
    by construction."""
    if is_propeller is None:
        is_propeller = propeller_mode(results_list)
    keys: list = []
    seen = set()
    for r in results_list:
        for k in r.summary.keys():
            if k not in seen:
                seen.add(k)
                keys.append(k)
    if "rpm" in seen and _rotor_rpm_is_redundant(results_list):
        seen.discard("rotor_rpm")
        keys = [k for k in keys if k != "rotor_rpm"]
    for k in [c for c in keys if not nomenclature.is_visible(c, is_propeller)]:
        seen.discard(k)
        keys = [c for c in keys if c != k]
    primary = [k for k in nomenclature.primary_order(is_propeller) if k in seen]
    primary += [k for k in _MAIN_COLUMNS
                if k in seen and k not in primary]
    cfg = [k for k in keys if k.startswith("cfg_")]
    remaining = [k for k in keys if k not in primary and k not in cfg]
    return primary + remaining, cfg


# =============================================================================
# `Results.summary` metadata: PUBLIC API
# =============================================================================
# `_COLUMN_SYMBOL`/`_COLUMN_UNITS`/`_MAIN_COLUMNS`/`_format_value`/
...
SUMMARY_SYMBOLS = _COLUMN_SYMBOL
SUMMARY_UNITS = _COLUMN_UNITS
SUMMARY_PRIMARY_KEYS = _MAIN_COLUMNS
format_summary_value = _format_value
summary_keys_union = _sorted_keys
...
def _summary_table(results_list: list, is_propeller: bool = False) -> str:
    ...
    """
    ordered, cfg = _sorted_keys(results_list, is_propeller)
    ...
    # `condition_labels`). It is the SAME label that goes into the file
    # name and, through it, into each figure's caption.
    labels = condition_labels(results_list)

    def cells(r, ks):
        return [_format_value(r.summary.get(k, "")) for k in ks]
    ...
    # distant blocks. That is exactly what a matrix solves in one row.
    #
    # The approximately 50 columns do not fit in 1600 px and should not: the horizontal
    ...
    html = _table_html(
        ["condition"] + ordered,
        [[label] + cells(r, ordered)
         for label, r in zip(labels, results_list)],
        is_propeller=is_propeller)
    if cfg:
        # The config echo is the only block that stays collapsed: it is about 46
...
def _report_metadata(project: Optional[Project], results_list: list) -> str:
    ...
    if project is not None:
        ...
        if twist:
            physics.append(("Twist (root &rarr; tip)", f"{twist[0]:.3g}&deg; &rarr; {twist[-1]:.3g}&deg;"))
        if chord:
            physics.append(("Chord c/R (root &rarr; tip)", f"{chord[0]:.3g} &rarr; {chord[-1]:.3g}"))

        if multi_section:
            physics.append(("Airfoil", f"multi-section ({len(project.airfoil_sections)} sections)"))
            physics.append(("Sections at r/R", ", ".join(
            ...
            if af.source == "analytical":
                ...
                physics.append(("Polar table", f"{len(slices)} slice(s)"))
            ...
            if af.use_dynamic_stall:
                physics.append(("Dynamic stall", f"&Oslash;ye, A={af.dynamic_stall_A:.3g}"))

    physics.append(("Conditions", str(len(results_list))))

    def _definition_list(block):
        return ('<dl class="meta">'
                + "".join(f"<dt>{k}</dt><dd>{v}</dd>" for k, v in block)
                + "</dl>")

    return ('<div class="meta-cols">' + _definition_list(general)
            + _definition_list(physics) + "</div>")
...
#: at about 1920 px; the report body has `max-width: 1600px`, so
#: 1920 px natively already gives approximately 1.2x oversampling (crisp on
...
#: out of the ordinary (see `_embedded_figure`).
_MAX_FIGURE_WIDTH = 2600
...
#: mathtext, because a `<figcaption>` caption does not render LaTeX.
_FIELD_LABELS = {
...
def _figure_caption(path: str) -> str:
    ...
    still without an entry in `_LABEL_BY_PREFIX`), it falls back to the
    raw filename instead of raising, because an ugly caption is a smaller problem
    than a report that fails to finish generating."""
    stem = Path(path).stem
    for prefix, label in _LABEL_BY_PREFIX:
        ...
        if prefix == "disk_map_":
            ...
            field = next((c for c in sorted(_FIELD_LABELS, key=len, reverse=True)
                          if rest == c or rest.startswith(c + "_")), None)
            if field is None:
                field, _, condition = rest.partition("_")
                field_label = field
            else:
                condition = rest[len(field):].lstrip("_")
                field_label = _FIELD_LABELS[field]
            text = f"{label}{field_label}"
        elif prefix == "coefficients_vs_":
            # `rest` IS the whole axis (for example "collective_deg"
            # has "_" in its own name). This file carries no condition
            ...
            condition = rest
        if condition:
            text += f" -- {_readable_condition(condition)}"
        return text
    return stem
...
def _readable_condition(condition: str) -> str:
    """Condition name coming from the FILE NAME, turned back into prose."""
    marked = condition
    for i, symbol in enumerate(sorted(_SUBSCRIPTED_SYMBOLS,
                                      key=len, reverse=True)):
        marked = marked.replace(symbol, f"\x00{i}\x00")
    marked = marked.replace("_", " ")
    for i, symbol in enumerate(sorted(_SUBSCRIPTED_SYMBOLS,
                                      key=len, reverse=True)):
        marked = marked.replace(f"\x00{i}\x00", symbol)
    return marked


def _embedded_figure(path: str) -> str:
    ...
    (about 204 KB for the 12-map panel) is SMALLER than the 1500 px JPEG
    (about 180 KB) per pixel delivered, and it is lossless.

    Now: below the limit, the original PNG passes through intact. Only above it
    (a really huge figure) is there resampling, and in PNG rather than JPEG, so as
    ...
    try:
        ...
        if im.width > _MAX_FIGURE_WIDTH:
            height = round(im.height * _MAX_FIGURE_WIDTH / im.width)
            im = im.resize((_MAX_FIGURE_WIDTH, height), Image.LANCZOS)
            buf = io.BytesIO()
...
#: figure, except the coefficient curve of a sweep). That is the
...
def _satellite_page(title: str, figures: list, master_name: str,
    ...
    return "\n".join([
        ...
        f"<title>{report_title} &mdash; {title}</title>",
        f"<style>{_REPORT_CSS}</style></head><body>",
        f'<p class="sub"><a href="{master_name}">&larr; back to the main report</a></p>',
        f"<h1>{title}</h1>",
        f'<p class="sub">{report_title} &mdash; {len(figures)} figure(s)</p>',
        *figures,
...
def _sort_by_condition(paths: list, results_list: list) -> list:
    ...
    has."""
    conditions_in_order = condition_filenames(results_list)

    def section_position(path: str) -> int:
        name = Path(path).stem
        # longest condition first: avoids "c1" matching inside
        # "c10" by mistake when both are valid condition names
        for i, cond in sorted(enumerate(conditions_in_order),
                              key=lambda pair: -len(pair[1])):
            if cond and name.endswith(cond):
                return i
        return len(conditions_in_order)   # no recognized condition: goes last

    return sorted(paths, key=section_position)


def _sort_disk_maps(paths: list, results_list: list) -> list:
    """Field by field first, the panel of 12 last; within each
    group, in `results_list` order (see `_sort_by_condition`)."""
    singles = [c for c in paths if "disk_map_grid" not in Path(c).name]
    grids = [c for c in paths if "disk_map_grid" in Path(c).name]
    return (_sort_by_condition(singles, results_list)
            + _sort_by_condition(grids, results_list))
...
def generate_report(results, path: str, *, project: Optional[Project] = None,
    ...
    ``project``: optional, but recommended, because it is what allows recording the
    ...
    Sections come out in a FIXED ORDER (``_REPORT_SECTIONS``), with a title
    ...
    if plots:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                written_files = export_results(results_list, tmp, plots=plots,
                ...
                for plot_kind in plots:
                    try:
                        written_files.extend(export_results(
            ...
            by_section: dict[str, list[str]] = {kind: []
                                                for kind, _ in _REPORT_SECTIONS}
            extras: list[str] = []
            for figure_file in (a for a in written_files if a.endswith(".png")):
                name = Path(figure_file).name
                if name.startswith("airfoil_"):
                    kind = "airfoil_polar"
                elif name.startswith("coefficients_vs_"):
                    kind = "coefficients"
                elif name.startswith("loads_vs_azimuth"):
                    kind = "azimuth"
                elif name.startswith("blade_loads_vs_span"):
                    kind = "span"
                elif name.startswith("disk_map_grid"):
                    # The consolidated panel belongs to the same section as the
                    # individual maps and must appear after them.
                    kind = "disk_map"
                elif name.startswith("disk_map_"):
                    kind = "disk_map"
                    ...
                    extras.append(figure_file)
                    continue
                by_section[kind].append(figure_file)

            for kind, title in _REPORT_SECTIONS:
                pngs = by_section[kind]
                if kind == "disk_map":
                    pngs = _sort_disk_maps(pngs, results_list)
                else:
                    pngs = _sort_by_condition(pngs, results_list)
                if pngs:
                    sections.append((title, [_embedded_figure(a) for a in pngs]))
            if extras:
    ...
    for stale in dest.parent.glob(f"{dest.stem}_*.html"):
        try:
            stale.unlink()
        except OSError:
            pass
    ...
    satellites: list[tuple[str, str, int]] = []      # (title, file name, number of figures)
    embedded: list[tuple[str, list[str]]] = []
    for title, figures in sections:
        if len(figures) > _FIGURES_BEFORE_OWN_FILE:
            name = f"{dest.stem}_{_section_slug(title)}.html"
            (dest.parent / name).write_text(
                _satellite_page(title, figures, dest.name, title),
                encoding="utf-8")
            satellites.append((title, name, len(figures)))
        else:
            embedded.append((title, figures))
