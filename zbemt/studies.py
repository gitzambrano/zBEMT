...
Orchestrates ``bemt.py`` across several flight conditions (single case,
sweeps, batch) and, in the future, ``external_solvers.py`` when the study
requires a polar generated by NeuralFoil. Never writes to disk — always
returns ``Results`` (or ``list[Results]``) in memory.

Called only by ``api.py`` (hard rule from plan Section 2: ``gui.py`` and
``main_batch.py`` do not know about this file).
"""
