...
- **SC-4** — The software shall provide equivalent functional access via graphical interface
...
- **SC-7** — The Geometry Designer shall evaluate and compare labeled blade planform variants
...
- **SC-7a** — Named geometry comparisons shall persist in `inputs/comparisons.bemt` with
  their constituent variants, evaluation flight conditions, and trim configurations.
- **SC-7b** — The Geometry Designer shall support explicit radial table overrides,
...
- **SC-7c** — Each comparison variant shall maintain a unique user-specified or
  parameter-derived identifier used across results tables, visualization plots, and data exports.
- **SC-7d** — Comparison results shall provide performance ranking metrics at each evaluated
  condition, defaulting to propulsive efficiency in propeller mode and figure of merit
  in rotor mode.
- **SC-7e** — Comparison outputs shall report performance deltas relative to the baseline
  planform using percentage changes for non-zero metrics and absolute differences for
  zero-crossing quantities.
- **SC-7f** — The comparison solver shall support constant-loading trim modes holding
  thrust or thrust coefficient ($C_T$) constant, where the baseline variant defines the
  target loading.
- **SC-7g** — Constant-loading trim shall converge via bisection, trimming rotational speed
  (RPM) in propeller mode and collective pitch in rotor mode.
- **SC-7h** — Trim targets outside the solvable physical bracket shall raise an explicit
  named error and shall not return unconverged or unphysical states.
- **SC-7i** — Trimmed comparison outputs shall record target loading, the trimmed degree of
  freedom, and the converged operating state.
- **SC-7j** — When a baseline blade lacks an analytical generator, planform scaling targets
  shall apply directly to the discrete radial distribution table.
- **SC-7k** — Root and tip targets shall rebuild chord and twist distributions to satisfy
  requested boundary values; normalized mean chord (`chord_norm`) and peak chord
  (`max_chord_norm`) shall scale the entire planform.
- **SC-7l** — Variant summaries, comparison tables, and CSV exports shall report root cutout,
  total radius, blade aspect ratio ($AR$), and rotor solidity ($\sigma$).
- **SC-7m** — The Geometry Designer shall allow importing an external project blade as an
  isolated comparison baseline without modifying the source project file.
- **SC-7n** — When a comparison variant defines distinct airfoil sections or blade dynamics,
  the output shall identify the comparison as non-geometry-only.
- **SC-7o** — Standard geometry-only comparisons shall enforce identical airfoil polars,
  radial discretization meshes, inflow models, and correction settings across all variants.
- **SC-7p** — Comparison evaluation conditions shall support project saved cases, explicit
  single flight conditions, or parametric sweeps across operating variables.
- **SC-8** — The optimization engine shall execute bounded single-objective planform
...
- **SC-8a** — Single-objective optimization searches shall initialize at bound midpoints,
  strictly enforce parameter boundaries, and penalize unconverged solver evaluations.
- **SC-8b** — Single-objective optimization capabilities shall be fully accessible through
  the CLI `--optimize` interface and Python library `api`.
- **SC-9** — The software shall support XFOIL as an external viscous airfoil polar
  computation engine.
- **SC-9a** — The XFOIL executable shall resolve sequentially from `ZBEMT_XFOIL_BIN`,
  the persisted GUI selection, system `PATH`, and standard platform install directories.
- **SC-9b** — XFOIL generation shall write dedicated execution scripts per Reynolds number
  and shall apply Prandtl-Glauert compressibility post-corrections.
- **SC-9c** — Viscous transition parameters (`ncrit`, `xtr_top`, `xtr_bot`) shall apply
  exclusively to XFOIL execution and shall be rejected by NeuralFoil.
- **SC-9d** — A missing XFOIL executable shall disable only XFOIL generation and shall raise
  a `RuntimeError` stating the cause and available remedies.
- **SC-10** — The airfoil geometry generator shall support standard profile parameterizations:
  NACA 4-digit, NACA 5-digit, CST, Bézier, PARSEC, Joukowski, biconvex, and imported coordinate contours.
- **SC-10a** — Each analytical airfoil family shall provide dedicated parameter editor controls
  in the GUI without requiring manual external file preparation.
- **SC-10b** — Profile generator parameters shall persist as structured `generator_params`
  blocks inside `inputs/airfoil.bemt`.
- **SC-11** — The dynamic solver shall compute periodic steady-state rigid-blade flapping and
...
- **SC-12** — The transient simulation engine shall time-march rotor operating states along
...
- **SC-16** — Tabulated airfoil polar definitions shall specify aerodynamic lift ($C_L$),
...
- **SC-16a** — A tabulated airfoil definition shall support radial variation across blade span
...
- **SC-16b** — Airfoil table import, CSV export, `.bemt` project serialization, and external
...
- **SC-5** — Free-wake, prescribed-wake, vortex-lattice inflow, and computational fluid
  dynamics (CFD) shall be out of scope.
- **SC-5a** — Supported inflow fields shall remain limited to annular-momentum and finite-state
  dynamic formulations.
- **SC-5b** — Modal and finite-element structural blade elasticity shall be out of scope; blade
  degrees of freedom shall remain rigid-body flapping and lagging under `SC-11`.
- **SC-6** — The core BEMT solver and CLI shall run without graphical dependencies (Qt or 3D
