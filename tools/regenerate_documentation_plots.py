...
PROJETO = REPO / "projects" / "starter_rotor"
SAIDA = REPO / "docs" / "img"
...
def _projeto(ne: int, npsi: int, **config):
    """Copy of starter_rotor with mesh and config overrides applied."""
    proj = deepcopy(api.open_project(str(PROJETO)))
    proj.config.update({"Ne": ne, "Npsi": npsi})
    proj.config.update(config)
    return proj


def _caso(proj, *, mu_x: float, collective_deg: float = 8.0, rpm: float = 1200.0,
          Vz: float = 0.0, nome: str = "case"):
    return api.run_case(proj, FlightCondition(
        name=nome, mu_x=mu_x, Vz=Vz, collective_deg=collective_deg, rpm=rpm))


def _salvar(fig, nome: str, *, tight: bool = True):
    if tight:
        fig.tight_layout()
    caminho = SAIDA / nome
...
def _indice_radial(maps, alvo: float) -> int:
    """Index of the radial station closest to ``alvo`` (in r/R)."""
    return int(np.argmin(np.abs(np.asarray(maps["r_norm_nodes"]) - alvo)))
...
_FRACAO_DE_SINAL_RELEVANTE = 0.02


def _norma_centrada_em_zero(Z):
    ...
    if zmin < 0.0 < zmax and abs(zmin) > _FRACAO_DE_SINAL_RELEVANTE * abs(zmax):
        return mcolors.TwoSlopeNorm(vmin=zmin, vcenter=0.0, vmax=zmax)
    return None


def _mapa_de_disco(ax, maps, campo, *, cmap="viridis", niveis=24, vmin=None, vmax=None,
                   norm=None, orientacao=False):
    ...
    Z = np.asarray(maps[campo]) if isinstance(campo, str) else np.asarray(campo)
    X = R * np.sin(PSI)
    Y = -R * np.cos(PSI)
    ...
    if norm is not None:
        cf = ax.contourf(X, Y, Z, levels=niveis, cmap=cmap, norm=norm)
    else:
        cf = ax.contourf(X, Y, Z, levels=niveis, cmap=cmap, vmin=vmin, vmax=vmax)
    r_raiz = float(np.min(R))
    ang = np.linspace(0, 2 * np.pi, 200)
    ax.add_patch(plt.Circle((0, 0), r_raiz, facecolor="white", edgecolor="none", zorder=3))
...
def fig_malha():
    ...
    (r, ψ)."""
    proj = api.open_project(str(PROJETO))
    r_raiz = float(proj.geometry.root_cutout_norm)
    ne, npsi = 12, 24

    r = np.linspace(r_raiz, 1.0, ne)
    psi = np.linspace(0.0, 2 * np.pi, npsi, endpoint=False)
    ang = np.linspace(0, 2 * np.pi, 400)

    fig, ax = plt.subplots(figsize=(6.4, 6.4))
    for ri in r:                                   # rings
        ax.plot(ri * np.cos(ang), ri * np.sin(ang), "-", color=AZUL, lw=0.5, alpha=0.55)
    for pj in psi:                                 # spokes
        ax.plot([r_raiz * np.cos(pj), np.cos(pj)], [r_raiz * np.sin(pj), np.sin(pj)],
                "-", color=AZUL, lw=0.5, alpha=0.55)
    ...
    ax.plot(r_raiz * np.cos(ang), r_raiz * np.sin(ang), "-", color="black", lw=1.6)
    ...
    _salvar(fig, "11-discretiza-o-do-disco-malha-radial-azimutal-1.png")
...
def fig_modelos_de_inflow():
    ...
    for modelo, titulo in modelos:
        proj = _projeto(NE_DISCO, NPSI_DISCO, inflow_field_model=modelo)
        res = _caso(proj, mu_x=MU_INFLOW, nome=modelo)
        campos.append((titulo, res))

    todos = np.concatenate([np.asarray(r.maps["lambda_i"]).ravel() for _t, r in campos])
    norm = _norma_centrada_em_zero(todos)

    fig, axes = plt.subplots(1, 4, figsize=(16.5, 4.6))
    cf = None
    for ax, (titulo, res) in zip(axes, campos):
        cf = _mapa_de_disco(ax, res.maps, "lambda_i", cmap="RdBu_r", niveis=22, norm=norm,
                            orientacao=True)
    ...
    fig.suptitle(r"Induced inflow field $\lambda_i(r,\psi)$ — "
                 ...
                 fontsize=11)
    _salvar(fig, "17-modelos-de-inflow-e-solvers-do-zbemt-1.png", tight=False)
...
def fig_convergencia_dos_solvers():
    ...
    proj = _projeto(NE_DISCO, NPSI_DISCO, collect_history=True)
    resultados = api.benchmark_solvers(
    ...
    cores = {"fixed_point": VERMELHO, "aitken": VERDE, "newton": AZUL, "bisection": "#e08a1e"}
    ...
    fig.suptitle(rf"Actual convergence of the 4 solvers — same rotor, $\mu_x={MU_SOLVERS}$",
                 fontsize=11)
    _salvar(fig, "17-modelos-de-inflow-e-solvers-do-zbemt-2.png")
...
def fig_curva_universal():
    ...
    vc_sobe = np.linspace(0.0, 4.0, 400)                 # climb/hover
    vi_sobe = -vc_sobe / 2 + np.sqrt((vc_sobe / 2) ** 2 + 1)
    ax.plot(vc_sobe, vi_sobe, "-", color=AZUL, lw=2.2,
            label="Climb / hover (momentum theory)")

    vc_moinho = np.linspace(-4.0, -2.0, 300)             # windmill brake
    vi_moinho = -vc_moinho / 2 - np.sqrt((vc_moinho / 2) ** 2 - 1)
    ax.plot(vc_moinho, vi_moinho, "--", color=AZUL, lw=2.2,
    ...
    vc_anel = np.linspace(-2.0, 0.0, 300)
    x = vc_anel / 2.0
    vi_anel = 1.0 + 1.10 * np.sin(np.pi * (vc_anel + 2.0) / 2.0) ** 1.6 * (1 - 0.18 * (x + 1) ** 2)
    ax.plot(vc_anel, vi_anel, ":", color=VERMELHO, lw=2.4,
            label="Vortex ring / turbulent wake\n(empirical fit — simple momentum invalid)")
    ax.axvspan(-2.0, 0.0, color=VERMELHO, alpha=0.06)
    ...
    _salvar(fig, "18-teoria-de-quantidade-de-movimento-1.png")
...
def _def_do_aerofolio():
    return deepcopy(api.open_project(str(PROJETO)).airfoil)
...
def fig_modelos_de_estol():
    defn = _def_do_aerofolio()
    alpha_deg = np.linspace(-40, 40, 601)
    alpha = np.deg2rad(alpha_deg)

    modelos = [
        ("linear", "linear (no stall)", CINZA, "--"),
        ("clip", "clip (abrupt saturation)", AZUL, "-"),
        ("enhanced", "enhanced (smooth decay)", VERMELHO, "-"),
    ]
    fig, (ax_l, ax_d) = plt.subplots(1, 2, figsize=(12.6, 4.8))
    for modelo, rotulo, cor, traco in modelos:
        cl, cd = _analitico(defn, modelo).cl_cd(alpha)
        ax_l.plot(alpha_deg, cl, traco, color=cor, lw=1.8, label=rotulo)
        ax_d.plot(alpha_deg, cd, traco, color=cor, lw=1.8, label=rotulo.split(" (")[0])
    for ax in (ax_l, ax_d):
    ...
    ax_l.set_title(r"AnalyticalAirfoil: linear vs. clip vs. enhanced — $C_l$")
    ax_d.set_title(r"$C_d$")
    _salvar(fig, "19-teoria-do-elemento-de-p-1.png")


def fig_tabela_de_aerofolio():
    ...
    tabela = airfoils.TableAirfoil(alpha_tab, cl_tab, cd_tab)
    cl_int, cd_int = tabela.cl_cd(np.deg2rad(alpha_deg))

    fig, (ax_l, ax_d) = plt.subplots(1, 2, figsize=(12.6, 4.8))
    for ax, ref, pts, interp, simbolo in (
        ...
        ax.plot(alpha_tab, pts, "o", color="#1f5c86", markersize=5,
                label="measured points" + (" (input data)" if simbolo == "C_l" else ""))
        ax.plot(alpha_deg, interp, "-", color=AZUL, lw=1.6,
                label="TableAirfoil" + (" (linear interpolation)" if simbolo == "C_l" else ""))
        ax.set_xlabel(r"$\alpha$ [deg]")
        ax.set_ylabel(rf"${simbolo}$")
        ax.legend()
    ax_l.set_title("TableAirfoil: linear interpolation between measured points")
    ax_d.set_title(r"$C_d$")
    _salvar(fig, "19-teoria-do-elemento-de-p-2.png")


def fig_viterna():
    defn = _def_do_aerofolio()
    base = _analitico(defn, "enhanced")
    estendido = airfoils.ViternaExtendedAirfoil(
        base,
        alpha_stall_pos_deg=defn.alpha_stall_pos_deg,
        alpha_stall_neg_deg=defn.alpha_stall_neg_deg,
    ...
    alpha = np.deg2rad(alpha_deg)
    cl_v, cd_v = estendido.cl_cd(alpha)
    ...
    for ax, naive, vit, simbolo in ((ax_l, cl_n, cl_v, "C_l"), (ax_d, cd_n, cd_v, "C_d")):
        ax.plot(alpha_deg, naive, "--", color=CINZA, lw=1.5,
                label="enhanced, naively extrapolated")
        ax.plot(alpha_deg, vit, "-", color=VERMELHO, lw=1.8,
                label="Viterna-Corrigan" + (" (-180° to 180°)" if simbolo == "C_l" else ""))
        ax.axvspan(defn.alpha_stall_neg_deg, defn.alpha_stall_pos_deg,
                   color=AZUL, alpha=0.10,
                   label="blended region (base model)" if simbolo == "C_l" else None)
        ax.set_xlabel(r"$\alpha$ [deg]")
        ax.set_ylabel(rf"${simbolo}$")
    ...
    _salvar(fig, "19-teoria-do-elemento-de-p-3.png")
...
def fig_carga_no_disco():
    proj = _projeto(NE_DISCO, NPSI_DISCO)
    res = _caso(proj, mu_x=MU_CARGA, nome="loads")

    fig, axes = plt.subplots(1, 2, figsize=(12.6, 5.4))
    for ax, campo, cmap, titulo in (
        ...
        norm = _norma_centrada_em_zero(np.asarray(res.maps[campo])) if campo == "Ft" else None
        cf = _mapa_de_disco(ax, res.maps, campo,
                            cmap=("RdBu_r" if norm is not None else cmap),
                            niveis=28, norm=norm, orientacao=True)
        ax.set_title(titulo)
        ax.set_xlabel(r"$x/R$")
        ax.set_ylabel(r"$y/R$")
        fig.colorbar(cf, ax=ax, fraction=0.046, pad=0.10)
    fig.suptitle(rf"Aerodynamic loading over the disk, $\mu_x={MU_CARGA}$ "
                 r"(advancing side $\psi=90°$ to the right)", fontsize=11)
    _salvar(fig, "19-teoria-do-elemento-de-p-4.png")
...
def fig_prandtl():
    ...
    _salvar(fig, "21-perda-de-prandtl-ponta-e-raiz-1.png")
...
def fig_mapa_de_ut():
    ...
    # Same convention as `_mapa_de_disco` (the engine's): psi=90° to the RIGHT.
    X, Y = RR * np.sin(PP), -RR * np.cos(PP)

    fig, ax = plt.subplots(figsize=(7.6, 5.4))
    cf = ax.contourf(X, Y, UT, levels=22, cmap="RdBu_r",
                     norm=_norma_centrada_em_zero(UT))
    ...
    _salvar(fig, "22-fluxo-reverso-cinco-modelos-1.png")


MU_REVERSO, R_REVERSO = 0.55, 0.30

def fig_modelos_de_fluxo_reverso():
    ...
    modelos = ["simple_flip", "flat_plate", "alpha_blending",
               "thin_plate_blend", "viterna_full_range"]
    cores = [VERMELHO, "#e08a1e", AZUL, VERDE, "#6a4c93"]

    fig, axes = plt.subplots(1, 3, figsize=(16.5, 4.8))
    sombreado = None
    for modelo, cor in zip(modelos, cores):
        proj = _projeto(NE_DISCO, NPSI_DISCO, reverse_flow_model=modelo)
        res = _caso(proj, mu_x=MU_REVERSO, nome=modelo)
        i = _indice_radial(res.maps, R_REVERSO)
        psi_deg = np.rad2deg(np.asarray(res.maps["psi_nodes"]))
        for ax, campo, escala in ((axes[0], "Cl", 1.0), (axes[1], "Cd", 1.0),
                                   (axes[2], "alpha_eff", np.rad2deg(1.0))):
            ax.plot(psi_deg, np.asarray(res.maps[campo])[i, :] * escala,
                    "-", color=cor, lw=1.5, label=modelo)
        if sombreado is None:
            ut = np.asarray(res.maps["Ut"])[i, :]
            reverso = ut < 0
            if reverso.any():
                sombreado = (psi_deg[reverso].min(), psi_deg[reverso].max())

    for ax, rotulo, titulo in ((axes[0], r"$C_l$", r"$C_l(\psi)$"),
                                (axes[1], r"$C_d$", r"$C_d(\psi)$"),
                                (axes[2], r"$\alpha_{eff}$ [deg]", r"$\alpha_{eff}(\psi)$")):
        if sombreado:
            ax.axvspan(*sombreado, color="0.85", zorder=0)
        ax.set_xlabel(r"Azimuth $\psi$ [deg]")
        ax.set_ylabel(rotulo)
        ax.set_title(titulo)
    axes[0].set_title(rf"$C_l(\psi)$  —  $r/R={R_REVERSO}$, $\mu_x={MU_REVERSO}$")
    ...
    _salvar(fig, "22-fluxo-reverso-cinco-modelos-2.png")
...
MU_SNEL, COLETIVO_SNEL = 0.05, 24.0
...
def fig_snel():
    ...
    stalled root."""
    sem = _caso(_projeto(NE_DISCO, NPSI_DISCO, use_rotational_augmentation=False),
                mu_x=MU_SNEL, collective_deg=COLETIVO_SNEL, nome="no_snel")
    com = _caso(_projeto(NE_DISCO, NPSI_DISCO, use_rotational_augmentation=True),
                mu_x=MU_SNEL, collective_deg=COLETIVO_SNEL, nome="snel")

    r = np.asarray(sem.maps["r_norm_nodes"])
    cl_sem = np.asarray(sem.maps["Cl"]).mean(axis=1)
    cl_com = np.asarray(com.maps["Cl"]).mean(axis=1)

    fig, (ax_a, ax_b) = plt.subplots(1, 2, figsize=(12.6, 4.6))
    ax_a.plot(r, cl_sem, "-o", color=AZUL, markersize=3.4, lw=1.4,
              label=r"without Snel ($C_{l,2D}$)")
    ax_a.plot(r, cl_com, "-s", color=VERMELHO, markersize=3.4, lw=1.4,
    ...
    ax_a.set_title(rf"Himmelskamp/Snel effect at $\mu_x={MU_SNEL}$, "
                   rf"$\theta_0={COLETIVO_SNEL:.0f}°$ (near hover, stalled root)")
    ax_a.legend()

    ax_b.plot(r, cl_com - cl_sem, "-^", color=VERDE, markersize=3.4, lw=1.4)
    ...
    _salvar(fig, "23-corre-es-f-sicas-locais-1.png")
...
def fig_fluxo_radial():
    sem = _caso(_projeto(NE_DISCO, NPSI_DISCO, use_radial_flow_correction=False),
                mu_x=MU_RADIAL, nome="no_radial")
    com = _caso(_projeto(NE_DISCO, NPSI_DISCO, use_radial_flow_correction=True),
                mu_x=MU_RADIAL, nome="radial")

    i = _indice_radial(sem.maps, R_RADIAL)
    r_real = float(np.asarray(sem.maps["r_norm_nodes"])[i])
    psi_deg = np.rad2deg(np.asarray(sem.maps["psi_nodes"]))
    cd_sem = np.asarray(sem.maps["Cd"])[i, :]
    cd_com = np.asarray(com.maps["Cd"])[i, :]

    fig, (ax_a, ax_b) = plt.subplots(1, 2, figsize=(12.6, 4.6))
    ax_a.plot(psi_deg, cd_sem, "-o", color=AZUL, markersize=3.0, lw=1.4,
              label="without radial correction")
    ax_a.plot(psi_deg, cd_com, "-s", color=VERMELHO, markersize=3.0, lw=1.4,
              label="with radial correction")
    ax_a.set_ylabel(r"$C_d$")
    ax_a.set_title(rf"$C_d(\psi)$ at $r/R\approx{r_real:.2f}$, $\mu_x={MU_RADIAL}$")
    ax_a.legend()

    ax_b.plot(psi_deg, cd_com - cd_sem, "-^", color=VERDE, markersize=3.0, lw=1.4)
    ax_b.axhline(0.0, color="black", lw=0.9)
    ...
    _salvar(fig, "23-corre-es-f-sicas-locais-2.png")
...
def fig_prandtl_glauert():
    ...
    fig, ax = plt.subplots(figsize=(7.0, 4.6))
    ax.plot(M, 1.0 / np.sqrt(1.0 - M ** 2), "-", color=VERMELHO, lw=2.0)
    ...
    _salvar(fig, "23-corre-es-f-sicas-locais-3.png")
...
def fig_transiente_pitt_peters():
    ...
    proj = _projeto(NE_DISCO, NPSI_DISCO, inflow_field_model="pitt_peters_steady")
    ...
    radial = airfoils.radial_reynolds_mach(rotor, cfg, mu_x=MU_DEGRAU)
    perfil = airfoils.to_blade_airfoil(proj.airfoil_sections or [proj.airfoil], radial=radial)

    grade = bemt._pitt_peters_geometry(rotor, cfg)
    r_nodes, psi_nodes, R_NORM, PSI, R_DIM, CHORD, THETA = grade

    nu_pairado, *_ = bemt._solve_pitt_peters_steady(
        rotor, perfil, cfg, MU_PAIRADO, 0.0,
        r_nodes, psi_nodes, R_NORM, PSI, R_DIM, CHORD, THETA)
    nu_final, *_ = bemt._solve_pitt_peters_steady(
        rotor, perfil, cfg, MU_DEGRAU, 0.0, r_nodes, psi_nodes, R_NORM, PSI, R_DIM, CHORD, THETA)

    dtau, n_passos = 0.02, 390
    nu = np.array(nu_pairado, dtype=float)
    tau, hist = [0.0], [nu.copy()]
    for k in range(n_passos):
        nu, _, _ = bemt._pitt_peters_exp_step(
            nu, dtau, rotor, perfil, cfg, MU_DEGRAU, 0.0,
            r_nodes, psi_nodes, R_NORM, PSI, R_DIM, CHORD, THETA)
        tau.append((k + 1) * dtau)
        hist.append(nu.copy())
    tau = np.asarray(tau)
    ...
    rotulos = [(0, r"$\nu_0$ (uniform)", VERMELHO),
               (1, r"$\nu_s$ (lateral)", AZUL),
               (2, r"$\nu_c$ (longitudinal)", VERDE)]
    fig, ax = plt.subplots(figsize=(7.6, 5.2))
    for idx, rotulo, cor in rotulos:
        ax.plot(tau, hist[:, idx], "-", color=cor, lw=1.9, label=rotulo)
        ax.axhline(nu_final[idx], color=cor, ls=":", lw=1.2)
    ...
    ax.set_title("Pitt-Peters transient response to a step in advance ratio\n"
                 rf"($\mu_x$: {MU_PAIRADO} → {MU_DEGRAU}, "
                 "zBEMT's actual exponential integrator)")
    ax.legend(loc="center right")
    _salvar(fig, "24-inflow-n-o-uniforme-1.png")
...
def fig_histerese_de_oye():
    ...
    alpha_deg = 9.0 + 9.0 * np.sin(psi)              # atravessa o estol 1x/rev
    alpha = np.deg2rad(alpha_deg)
    ...
    casos = [(0.12, "low $c/R$ (quasi-steady)", VERDE),
             (0.50, "medium $c/R$ (representative)", AZUL),
             (1.25, "high $c/R$ (strongly dynamic)", VERMELHO)]
    ...
    for tau_omega, rotulo, cor in casos:
        ...
        ax_l.plot(alpha_deg, cl_dyn, "-", color=cor, lw=1.6,
                  label=rf"{rotulo} ($\tau\Omega$={tau_omega:.2f} rad)")
        ax_f.plot(np.rad2deg(psi), f, "-", color=cor, lw=1.6, label=rotulo)
    ...
    ax_f.set_title(r"Separation function $f(\psi)$ — relaxation towards $f_{st}$")
    ax_f.legend(fontsize=8, loc="lower right")
    _salvar(fig, "25-estol-din-mico-de-ye-1.png")


# =============================================================================
# 12.2 / 12.3 -- Varreduras em mu_x (figs. 28-1, 29-1)
# =============================================================================

def _varredura(mu_max: float, n: int):
    proj = _projeto(NE_VARREDURA, NPSI_VARREDURA)
    mus = np.linspace(0.01, mu_max, n)
    return mus, [_caso(proj, mu_x=float(m), nome=f"mu_{m:.3f}").summary for m in mus]


def fig_varredura_ct_cq_fm():
    mus, resumos = _varredura(0.45, 20)
    ct = [s["CT"] for s in resumos]
    cq = [s["CQ"] for s in resumos]
    fm = [s.get("FM", np.nan) for s in resumos]

    fig, axes = plt.subplots(1, 3, figsize=(14.5, 4.3))
    for ax, y, cor, rotulo, titulo in (
        ...
        ax.plot(mus, y, "-o", color=cor, markersize=4, lw=1.4)
        ax.set_xlabel(r"$\mu_x$")
        ax.set_ylabel(rotulo)
        ax.set_title(titulo)
    _salvar(fig, "28-agrega-o-de-resultados-1.png")


def fig_varredura_ct_cp_fm():
    mus, resumos = _varredura(0.35, 24)
    ct = [s["CT"] for s in resumos]
    cp = [s["CP"] for s in resumos]
    fm = [s.get("FM", np.nan) for s in resumos]

    fig, (ax_c, ax_f) = plt.subplots(1, 2, figsize=(12.6, 4.6))
    ax_c.plot(mus, ct, "-o", color=VERMELHO, markersize=3.6, lw=1.5, label=r"$C_T$")
    ax_c.plot(mus, cp, "-o", color=AZUL, markersize=3.6, lw=1.5, label=r"$C_P$")
    ax_c.set_xlabel(r"$\mu_x$")
    ax_c.set_ylabel("coefficient")
    ax_c.set_title(r"Actual sweep in $\mu_x$ (fixed pitch, no trim)")
    ax_c.legend()

    ax_f.plot(mus, fm, "-o", color=VERDE, markersize=3.6, lw=1.5)
    ax_f.set_xlabel(r"$\mu_x$")
    ax_f.set_ylabel(r"$FM$")
    ax_f.set_title("Figure of merit along the sweep")
    _salvar(fig, "29-varreduras-de-condi-o-de-voo-1.png")
...
FIGURAS = [
    ("11-1 mesh", fig_malha),
    ("17-1 inflow models", fig_modelos_de_inflow),
    ("17-2 solver convergence", fig_convergencia_dos_solvers),
    ("18-1 universal inflow curve", fig_curva_universal),
    ("19-1 stall models", fig_modelos_de_estol),
    ("19-2 table airfoil", fig_tabela_de_aerofolio),
    ("19-3 viterna", fig_viterna),
    ("19-4 disk loads", fig_carga_no_disco),
    ("21-1 prandtl loss", fig_prandtl),
    ("22-1 Ut map", fig_mapa_de_ut),
    ("22-2 reverse flow models", fig_modelos_de_fluxo_reverso),
    ("23-1 snel", fig_snel),
    ("23-2 radial flow", fig_fluxo_radial),
    ("23-3 prandtl-glauert", fig_prandtl_glauert),
    ("24-1 pitt-peters transient", fig_transiente_pitt_peters),
    ("25-1 oye hysteresis", fig_histerese_de_oye),
    ("28-1 sweep CT/CQ/FM", fig_varredura_ct_cq_fm),
    ("29-1 sweep CT/CP + FM", fig_varredura_ct_cp_fm),
]


def main():
    SAIDA.mkdir(parents=True, exist_ok=True)
    for rotulo, funcao in FIGURAS:
        print(f"[{rotulo}]")
        funcao()
    print(f"\n{len(FIGURAS)} figures written to {SAIDA}")
