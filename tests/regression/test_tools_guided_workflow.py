...
class TestGuidedToolsUX(unittest.TestCase):
    ...
    def test_launcher_is_task_oriented(self):
        ...
        visible_copy = " ".join(
            w.text() for w in launcher.findChildren(QLabel))
    ...
    def test_designer_condition_default_matches_visible_panel(self):
        window = GeometryDesignerWindow(AppState())
        self.addCleanup(window.deleteLater)
        self.assertTrue(window.radio_single.isChecked())
        self.assertEqual(window.conditions_stack.currentIndex(), 1)

    def test_optimizer_advanced_rows_really_collapse(self):
        window = OptimizerWindow(AppState())
        self.addCleanup(window.deleteLater)
        self.assertFalse(window.advanced_search_box.isChecked())
        self.assertTrue(all(w.isHidden()
                            for w in window._advanced_search_widgets))
        window.advanced_search_box.setChecked(True)
        self.assertTrue(all(not w.isHidden()
                            for w in window._advanced_search_widgets))

    def test_stability_default_preset_is_applied(self):
        window = StabilityWindow(AppState())
        self.addCleanup(window.deleteLater)
        self.assertEqual(window.preset_combo.currentData(), "longitudinal")
        self.assertTrue(window.var_checks["u"].isChecked())
        self.assertTrue(window.var_checks["w"].isChecked())
        self.assertTrue(window.var_checks["theta_0"].isChecked())
        self.assertFalse(window.var_checks["v"].isChecked())

    def test_workflow_guidance_has_explicit_contrast(self):
        window = GeometryDesignerWindow(AppState())
        self.addCleanup(window.deleteLater)
        guidance = window.workflow_header.guidance
        style = guidance.styleSheet().lower()
        self.assertIn("background-color:", style)
        self.assertIn("color:", style)
        self.assertIn("padding:", style)
        self.assertIn("do this now", guidance.text().lower())
