...
class TestBisectionSolverContract(unittest.TestCase):
    ...
    def test_bisection_selects_the_root_nearest_the_physical_initial_guess(self):
        """A nonlinear element can have several mathematical inflow roots."""
        initial = np.array([[0.38]])
        cfg = BEMTConfig(
            solver="bisection", max_iter=100, tol=1e-10,
            early_exit_fraction=1.0,
        )
        ...
        def residual_fn(lambda_i):
            residual = -(
                (lambda_i + 0.2) * (lambda_i - 0.1) * (lambda_i - 0.4)
            )
            return {"lambda_i_next": lambda_i + residual}

        lambda_i, _state, converged, *_ = solve_bisection(
            residual_fn, initial, cfg)

        self.assertTrue(np.all(converged))
        self.assertAlmostEqual(float(lambda_i[0, 0]), 0.4, places=8)
