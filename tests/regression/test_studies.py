...
class TestRunCaseTrimmed(unittest.TestCase):
    ...
    def test_solve_collective_hits_target_thrust(self):
        ...
        self.assertEqual(res.summary["trim_target"], target)
        self.assertEqual(res.summary["trim_dof"], "collective_deg")
        self.assertTrue(res.summary["trim_converged"])
        self.assertLessEqual(res.summary["trim_residual"], abs(target) * 1e-4)
    ...
    def test_bisection_iteration_exhaustion_is_recorded(self):
        def linear_result(_project, condition, should_cancel=None):
            value = float(condition.collective_deg)
            return Results(summary={
                "Thrust": value,
                "CT": value,
                "collective_deg": value,
                "rpm": condition.rpm,
            })

        condition = FlightCondition(name="limited", rpm=600.0)
        with unittest.mock.patch.object(studies, "run_single_case", linear_result):
        ...
        self.assertEqual(result.summary["trim_target"], 1.0)
        self.assertEqual(result.summary["trim_dof"], "collective_deg")
        self.assertEqual(result.summary["trim_residual"], 1.0)
        self.assertFalse(result.summary["trim_converged"])

    def test_three_dof_trim_records_target_kind_without_name_error(self):
        summary = {
            "collective_deg": 8.0,
            "cyclic_c_deg": 0.1,
            "cyclic_s_deg": -0.1,
        ...
        solved = Results(summary=summary)
        project = _make_project()
        project.geometry.dynamics.flap_model = "offset"
        with unittest.mock.patch.object(studies, "_trim_by_newton", return_value=solved):
            result = studies.run_case_trimmed(
                project,
                FlightCondition(rpm=600.0),
                trim_mode="solve_collective_and_cyclic",
                target_kind="thrust",
                target_value=300.0,
            )
        self.assertEqual(result.summary["trim_target_kind"], "thrust")
