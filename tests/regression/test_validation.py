...
class TestValidateConfig(unittest.TestCase):
    def test_nonphysical_atmosphere_and_mesh_values_are_errors(self):
        invalid_values = {
            "rho": -1.0,
            "a_sound": 0.0,
            "nu_air": -1e-5,
        ...
        for field, value in invalid_values.items():
            with self.subTest(field=field):
                ...
                self.assertTrue(
                    any(issue.level == "error" and field in issue.message for issue in issues),
                    str(issues),
                )
...
class TestValidateBladeDynamics(unittest.TestCase):
    def test_rigid_flap_cannot_silently_discard_enabled_lead_lag(self):
        from zbemt.models import BladeDynamicsDef, RotorGeometryDef

        dynamics = BladeDynamicsDef(
            flap_model="rigid",
            lag_enabled=True,
            lag_inertia_kg_m2=100.0,
            lag_spring_nm_per_rad=8000.0,
        )
        issues = validation.validate_blade_dynamics(
            dynamics,
            RotorGeometryDef(),
            rho=1.225,
            cl_alpha=2.0 * 3.141592653589793,
            rpm=400.0,
        )
        self.assertTrue(
            any(issue.level == "error" and "rigid" in issue.message.lower()
                and "lead-lag" in issue.message.lower() for issue in issues),
            str(issues),
        )
